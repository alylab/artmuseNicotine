...
coefs_aprime_by_ppm <- aprime_by_ppm_boot %>%
  mutate(coefs = map2(coefs, coefs_raw, ~full_join(.x, .y, by = "term"))) %>%
  select(iteration, metric_type, exptCond, probe, coefs) %>%
  unnest(coefs) %>%
  rename(raw = "estimate_raw", perf_metric_type = "metric_type") %>%
  filter(term == "ppm_diff") %>%
  summarize_report(estimate, perf_metric_type, exptCond, probe, raw,
