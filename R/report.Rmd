...
coefs_aprime_by_ppm <- aprime_by_ppm_boot %>%
  select(iteration, metric_type, exptCond, probe, starts_with("coefs")) %>%
  pivot_longer(cols = starts_with("coefs"), names_to = c("model_type", ".value"), names_pattern = c("coefs_(.*)_(.*)")) %>% 
  mutate(raw = map(raw, ~rename_if(.x, is.numeric, ~paste0(., "_raw")))) %>% 
  mutate(coefs = map2(boot, raw, ~full_join(.x, .y, by = "term"))) %>% 
  select(-boot, -raw) %>% 
  unnest(coefs) %>%
...
pcor_aprime_by_ppm <- aprime_by_ppm_boot %>%
  select(iteration, metric_type, exptCond, probe, starts_with("resid")) %>%
  unnest(c(resid_value_diff, resid_ppm_diff)) %>%
  group_by(iteration, metric_type, exptCond, probe) %>%
  summarize(pcor = cor(value_diff_resid, ppm_diff_resid)) %>%
  rename(perf_metric_type = "metric_type") %>%
  summarize_report(pcor, perf_metric_type, exptCond, probe, stat_name = "partial Pearson correlation") %>%
  select(-metric_type) %>% 
  rename(metric_type = "perf_metric_type") %>% 
...
pcor_diff_aprime_by_ppm <- aprime_by_ppm_boot %>%
  ...
  rename(perf_metric_type = "metric_type") %>%
  summarize_report(pcor_diff, perf_metric_type, stat_name = "partial Pearson's r difference")
```
